...
public class Bullet14Trigger : MonoBehaviour
{
     ...
     void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "Enemy")
        {
            ...
            GameObject target = GetComponent<BulletState>().Target;
            ...
            Collider[] hits = Physics.OverlapSphere(target.transform.position,1.5f);
            if (hits.Length > 0)
            {
                for (int i = 0; i < hits.Length; i++)
                    if (hits[i].gameObject.tag == "Enemy")
                    {
                        hits[i].gameObject.GetComponent<EnemyStat>().DamageTrigger(tower, tower.GetComponent<TowerStat>().DamageF());
